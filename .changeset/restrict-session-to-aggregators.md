...
- `zeroEx()` authorises every call shape a 0x swap can take — the direct `AllowanceHolder.exec` call and the Rhinestone-Swapper-wrapped one — because the orchestrator picks between them after the session is signed. Narrow with `shape: 'direct' | 'wrapped'` when the shape is known. `fynd()`, `rhinestoneSwap()` and `swapperZeroEx()` cover the other venues, the aggregator-agnostic Swapper route, and the Swapper route pinned to 0x.
